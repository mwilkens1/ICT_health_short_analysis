...
#long working days
EWCS_EU28$long_wd <- ordered(EWCS_EU28$long_wd)
#y15_q37dr
...
Quantitative demands index. Following the 6th EWCS overview report. The fit seems to be very good if allowing for covariance between Q49a and b. The argument is that these questions are asked in the same way after one another so that could cause straightlining. 

```{r}

model <- 'QD =~ y15_Q49a + y15_Q49b + y15_Q61g + freq_dis_int

          y15_Q49a ~~ y15_Q49b

'  

fit <- sem(model, data=subset(EWCS_EU28,year>=2005))
summary(fit, fit.measures=TRUE, rsquare=TRUE, standardized =TRUE)

```

Using multiple waves of the survey may cause measurement invariance. The increase in the fit measures due to assuming measurement invariance are minimal, so safe to assume. 

```{r}

measurementInvariance(model= model, 
                      data = subset(EWCS_EU28,year>=2005), 
                      group = "wave")

```

Autonomy, based on the EWCS6 overview report. Very good fit.

```{r}

model <- 'AUT =~ y15_Q54a + y15_Q54b + y15_Q54c + y15_Q61e '  
