...
ggsave("Quantitative demands by computer use.eps", 
       plot = p + labs(title=NULL,caption=NULL), 
       device = "eps", path="charts/",
...
ggsave("Autonomy by computer use.eps", 
       plot = p + labs(title=NULL,caption=NULL), 
       device = "eps", path="charts/",
       scale = 1, width = 8, height = 6, units = "cm")
...
ggsave("Cognitive demands by computer use.eps", 
       plot = p + labs(title=NULL,caption=NULL), 
       device = "eps", path="charts/",
       scale = 1, width = 8, height = 6, units = "cm")
...
ggsave("Social support by computer use.eps", 
       plot = p + labs(title=NULL,caption=NULL), 
       device = "eps", path="charts/",
       scale = 1, width = 8, height = 6, units = "cm")
...
ggsave("Working hours by computer use.eps", 
       plot = p + labs(title=NULL,caption=NULL), 
       device = "eps", path="charts/",
...
ggsave("Frequent disruptive interruptions by computer use.eps", 
       plot = p + labs(title=NULL,caption=NULL), 
       device = "eps", path="charts/",
       scale = 1, width = 8, height = 5, units = "cm")
...
ggsave("Working in the free time for work demands by computer use.eps", 
       plot = p + labs(title=NULL,caption=NULL), 
       device = "eps", path="charts/",
       scale = 1, width = 8, height = 5, units = "cm")
...
ggsave("Ability to take an hour off by computer use.eps", 
       plot = p + labs(title=NULL,caption=NULL), 
       device = "eps", path="charts/",
       scale = 1, width = 8, height = 5, units = "cm")
...
ggsave("WHO5 by computer use.eps", 
       plot = p + labs(title=NULL,caption=NULL), 
       device = "eps", path="charts/",
       scale = 1, width = 8, height = 5, units = "cm")
write.csv(df, file="charts/WHO5 by computer use.csv")
...
df$goodhealth <- as.numeric(df$y15_Q75=="Very good                   " | df$y15_Q75=="Good                        ")

svydes <- svydesign(ids=~1, data=df, weights=~w5_EU28)

df1 <- svyby(~goodhealth, ~y15_Q30i, svydes, svymean, na.rm = TRUE)
df1 <- cbind(df1, confint(df1))
df1$value <- df1$goodhealth 
df1$variable <- "Good health"
df <- df1[,-2]

p <- ggplot(df, aes(x=y15_Q30i, y=value)) +
      geom_errorbar(aes(ymin=`2.5 %`, ymax=`97.5 %`), width=0.3, colour=EF_blue) +
      geom_point(size=2, colour=EF_blue) +
      scale_y_continuous(expand = c(0, 0), limit = c(0.7,0.9), breaks = seq(0.7, 0.9, 0.05), labels = percent_format(accuracy = 1)) +
      coord_flip() +
      labs(title = "Good self reported health by computer use",
           caption = "Source: EWCS, EU28 in 2015. Bars indicate 95% confidence interval.",
           x="Computer use",
           y=NULL) +
      theme_minimal(base_size = 10) +
...
ggsave("Good self reported health by computer use.eps", 
       plot = p + labs(title=NULL,caption=NULL), 
       device = "eps", path="charts/",
       scale = 1, width = 8, height = 5, units = "cm")
write.csv(df, file="charts/Good self reported health by computer use.csv")
...
ggsave("Absenteeism and presenteeism by computer use.eps", 
       plot = p + labs(title=NULL,caption=NULL), 
       device = "eps", path="charts/",
       scale = 1, width = 8, height = 6, units = "cm")
...
ggsave("Headaches or eystrain by computer use.eps", 
       plot = p + labs(title=NULL,caption=NULL), 
       device = "eps", path="charts/",
       scale = 1, width = 8, height = 5, units = "cm")
...
ggsave("Anxiety, stress and fatigue by computer use.eps", 
       plot = p + labs(title=NULL,caption=NULL), 
       device = "eps", path="charts/",
       scale = 1, width = 8, height = 6, units = "cm")
...
p <-  ggplot(subset(df2,x=="Indirect" | x=="Direct"), aes(x=Outcome, y=est.std, fill=x)) + 
...
ggsave("Effect of computer use on health indicators.eps", 
       plot = p + labs(title=NULL,caption=NULL, subtitle=NULL), 
       device = "eps", path="charts/",
       scale = 1, width = 16, height = 6, units = "cm")
write.csv(df, file="charts/Effect of computer use on health indicators.csv")
...
p <-  ggplot(subset(df2,is.na(x) ), aes(x=Mediator, y=est.std, fill=x2)) + 
        facet_wrap( ~ Outcome , nrow=2  ) +
        geom_bar(stat="identity", fill=EF_orange) + 
        geom_errorbar(aes(ymin=ci.lower, ymax=ci.upper), colour="black", width=.3, size=0.2) +
        scale_y_continuous(expand = c(0, 0), limit = c(-0.04,0.05), breaks = seq(-0.04, 0.04, 0.02),labels = number_format(accuracy=0.01)) +
        ...
        theme(panel.border = element_rect(fill=NA, colour= "grey"),
              strip.background = element_rect(fill="grey88", colour="grey88"),
              axis.title.y = element_blank(),
              axis.title.x = element_text(size=8),
              axis.text.x = element_text(size=6))
p

ggsave("Effect of ICT use mediated through job demands and resources.eps", 
       plot = p + labs(title=NULL,caption=NULL, subtitle=NULL), 
       device = "eps", path="charts/",
       scale = 1, width = 16, height = 8, units = "cm")
write.csv(subset(df2,is.na(x) ), file="charts/Effect of ICT use mediated through job demands and resources.csv")
...
df3$rhs[df3$rhs=="CD"] <- "Cognitive demands"
...
df3$rhs <- factor(df3$rhs, levels=c("Weekly working hours","Work in free time","Quantitative demands","Frequent disruptive interruptions",
                                    "Take hour off","Cognitive demands","Autonomy","Social support","Computer"))
df3$rhs <- factor(df3$rhs, levels=rev(levels(df3$rhs)))

p <-  ggplot(df3, aes(x=rhs, y=est.std)) + 
        facet_wrap(~ lhs, nrow=2) +
        geom_bar(stat="identity", fill=EF_green) + 
        geom_errorbar(aes(ymin=ci.lower, ymax=ci.upper), colour="black", width=.2, size=0.2) +
        coord_flip() + 
...
ggsave("Job demands and resources and health outcomes.eps", 
       plot = p + labs(title=NULL,caption=NULL, subtitle=NULL), 
       device = "eps", path="charts/",
       scale = 1, width = 16, height = 8, units = "cm")
write.csv(df3, file="charts/Job demands and resources and health outcomes.csv")
...
EWCS_EU28$alwaysoffice_1 <- as.numeric(EWCS_EU28$TICTM2=="Always at employers premise w/o ICT, employee")
EWCS_EU28$alwaysofficeICT_1  <- as.numeric(EWCS_EU28$TICTM2=="Always at employers premise with ICT, employee" )
EWCS_EU28$teleworker_1  <- as.numeric(EWCS_EU28$TICTM2=="Home-based teleworker, employee" )
EWCS_EU28$highmob_1  <- as.numeric(EWCS_EU28$TICTM2=="High mobile T/ICTM, employee")
EWCS_EU28$lowmob_1 <- as.numeric( EWCS_EU28$TICTM2=="Occasional T/ICTM, employee")
EWCS_EU28$self_alwaysoffice <- as.numeric(EWCS_EU28$TICTM2=="Self-employed")
EWCS_EU28$selfICTM <- as.numeric(EWCS_EU28$TICTM2=="Self-employed T/ICTM")
...
sm9  <- '#Measurement models
         ...
         #Indirect effects
           ...
           QD0_WHO5         := a0i0*a1 + a0i1*a1 + a0i2*a1 + a0i3*a1 + a0i4*a1 + a0i5*a1 + a0i6*a1
           QD0_y15_Q75      := a0i0*a2 + a0i1*a2 + a0i2*a2 + a0i3*a2 + a0i4*a2 + a0i5*a2 + a0i6*a2
           QD0_anxiety      := a0i0*a3 + a0i1*a3 + a0i2*a3 + a0i3*a3 + a0i4*a3 + a0i5*a3 + a0i6*a3
           QD0_fatigue      := a0i0*a4 + a0i1*a4 + a0i2*a4 + a0i3*a4 + a0i4*a4 + a0i5*a4 + a0i6*a4
           QD0_stress       := a0i0*a5 + a0i1*a5 + a0i2*a5 + a0i3*a5 + a0i4*a5 + a0i5*a5 + a0i6*a5
           QD0_absenteeism  := a0i0*a6 + a0i1*a6 + a0i2*a6 + a0i3*a6 + a0i4*a6 + a0i5*a6 + a0i6*a6
           QD0_presenteeism := a0i0*a7 + a0i1*a7 + a0i2*a7 + a0i3*a7 + a0i4*a7 + a0i5*a7 + a0i6*a7
           QD0_headeye      := a0i0*a8 + a0i1*a8 + a0i2*a8 + a0i3*a8 + a0i4*a8 + a0i5*a8 + a0i6*a8

           AUT0_WHO5         := b0i0*b1 + b0i1*b1 + b0i2*b1 + b0i3*b1 + b0i4*b1 + b0i5*b1 + b0i6*b1 
           AUT0_y15_Q75      := b0i0*b2 + b0i1*b2 + b0i2*b2 + b0i3*b2 + b0i4*b2 + b0i5*b2 + b0i6*b2
           AUT0_anxiety      := b0i0*b3 + b0i1*b3 + b0i2*b3 + b0i3*b3 + b0i4*b3 + b0i5*b3 + b0i6*b3
           AUT0_fatigue      := b0i0*b4 + b0i1*b4 + b0i2*b4 + b0i3*b4 + b0i4*b4 + b0i5*b4 + b0i6*b4
           AUT0_stress       := b0i0*b5 + b0i1*b5 + b0i2*b5 + b0i3*b5 + b0i4*b5 + b0i5*b5 + b0i6*b5
           AUT0_absenteeism  := b0i0*b6 + b0i1*b6 + b0i2*b6 + b0i3*b6 + b0i4*b6 + b0i5*b6 + b0i6*b6
           AUT0_presenteeism := b0i0*b7 + b0i1*b7 + b0i2*b7 + b0i3*b7 + b0i4*b7 + b0i5*b7 + b0i6*b7
           AUT0_headeye      := b0i0*b8 + b0i1*b8 + b0i2*b8 + b0i3*b8 + b0i4*b8 + b0i5*b8 + b0i6*b8

           CD0_WHO5         := c0i0*c1 + c0i1*c1 + c0i2*c1 + c0i3*c1 + c0i4*c1 + c0i5*c1 + c0i6*c1
           CD0_y15_Q75      := c0i0*c2 + c0i1*c2 + c0i2*c2 + c0i3*c2 + c0i4*c2 + c0i5*c2 + c0i6*c2
           CD0_anxiety      := c0i0*c3 + c0i1*c3 + c0i2*c3 + c0i3*c3 + c0i4*c3 + c0i5*c3 + c0i6*c3
           CD0_fatigue      := c0i0*c4 + c0i1*c4 + c0i2*c4 + c0i3*c4 + c0i4*c4 + c0i5*c4 + c0i6*c4
           CD0_stress       := c0i0*c5 + c0i1*c5 + c0i2*c5 + c0i3*c5 + c0i4*c5 + c0i5*c5 + c0i6*c5
           CD0_absenteeism  := c0i0*c6 + c0i1*c6 + c0i2*c6 + c0i3*c6 + c0i4*c6 + c0i5*c6 + c0i6*c6
           CD0_presenteeism := c0i0*c7 + c0i1*c7 + c0i2*c7 + c0i3*c7 + c0i4*c7 + c0i5*c7 + c0i6*c7
           CD0_headeye      := c0i0*c8 + c0i1*c8 + c0i2*c8 + c0i3*c8 + c0i4*c8 + c0i5*c8 + c0i6*c8

           SS0_WHO5         := d0i0*d1 + d0i1*d1 + d0i2*d1 + d0i3*d1 + d0i4*d1 + d0i5*d1 + d0i6*d1
           SS0_y15_Q75      := d0i0*d2 + d0i1*d2 + d0i2*d2 + d0i3*d2 + d0i4*d2 + d0i5*d2 + d0i6*d2
           SS0_anxiety      := d0i0*d3 + d0i1*d3 + d0i2*d3 + d0i3*d3 + d0i4*d3 + d0i5*d3 + d0i6*d3
           SS0_fatigue      := d0i0*d4 + d0i1*d4 + d0i2*d4 + d0i3*d4 + d0i4*d4 + d0i5*d4 + d0i6*d4
           SS0_stress       := d0i0*d5 + d0i1*d5 + d0i2*d5 + d0i3*d5 + d0i4*d5 + d0i5*d5 + d0i6*d5
           SS0_absenteeism  := d0i0*d6 + d0i1*d6 + d0i2*d6 + d0i3*d6 + d0i4*d6 + d0i5*d6 + d0i6*d6
           SS0_presenteeism := d0i0*d7 + d0i1*d7 + d0i2*d7 + d0i3*d7 + d0i4*d7 + d0i5*d7 + d0i6*d7
           SS0_headeye      := d0i0*d8 + d0i1*d8 + d0i2*d8 + d0i3*d8 + d0i4*d8 + d0i5*d8 + d0i6*d8

           freq_dis_int0_WHO5         := e0i0*e1 + e0i1*e1 + e0i2*e1 + e0i3*e1 + e0i4*e1 + e0i5*e1 + e0i6*e1
           freq_dis_int0_y15_Q75      := e0i0*e2 + e0i1*e2 + e0i2*e2 + e0i3*e2 + e0i4*e2 + e0i5*e2 + e0i6*e2
           freq_dis_int0_anxiety      := e0i0*e3 + e0i1*e3 + e0i2*e3 + e0i3*e3 + e0i4*e3 + e0i5*e3 + e0i6*e3
           freq_dis_int0_fatigue      := e0i0*e4 + e0i1*e4 + e0i2*e4 + e0i3*e4 + e0i4*e4 + e0i5*e4 + e0i6*e4
           freq_dis_int0_stress       := e0i0*e5 + e0i1*e5 + e0i2*e5 + e0i3*e5 + e0i4*e5 + e0i5*e5 + e0i6*e5
           freq_dis_int0_absenteeism  := e0i0*e6 + e0i1*e6 + e0i2*e6 + e0i3*e6 + e0i4*e6 + e0i5*e6 + e0i6*e6
           freq_dis_int0_presenteeism := e0i0*e7 + e0i1*e7 + e0i2*e7 + e0i3*e7 + e0i4*e7 + e0i5*e7 + e0i6*e7
           freq_dis_int0_headeye      := e0i0*e8 + e0i1*e8 + e0i2*e8 + e0i3*e8 + e0i4*e8 + e0i5*e8 + e0i6*e8

           totalhour0_WHO5         := f0i0*f1 + f0i1*f1 + f0i2*f1 + f0i3*f1 + f0i4*f1 + f0i5*f1 + f0i6*f1
           totalhour0_y15_Q75      := f0i0*f2 + f0i1*f2 + f0i2*f2 + f0i3*f2 + f0i4*f2 + f0i5*f2 + f0i6*f2
           totalhour0_anxiety      := f0i0*f3 + f0i1*f3 + f0i2*f3 + f0i3*f3 + f0i4*f3 + f0i5*f3 + f0i6*f3
           totalhour0_fatigue      := f0i0*f4 + f0i1*f4 + f0i2*f4 + f0i3*f4 + f0i4*f4 + f0i5*f4 + f0i6*f4
           totalhour0_stress       := f0i0*f5 + f0i1*f5 + f0i2*f5 + f0i3*f5 + f0i4*f5 + f0i5*f5 + f0i6*f5
           totalhour0_absenteeism  := f0i0*f6 + f0i1*f6 + f0i2*f6 + f0i3*f6 + f0i4*f6 + f0i5*f6 + f0i6*f6
           totalhour0_presenteeism := f0i0*f7 + f0i1*f7 + f0i2*f7 + f0i3*f7 + f0i4*f7 + f0i5*f7 + f0i6*f7
           totalhour0_headeye      := f0i0*f8 + f0i1*f8 + f0i2*f8 + f0i3*f8 + f0i4*f8 + f0i5*f8 + f0i6*f8

           y15_Q470_WHO5         := g0i0*g1 + g0i1*g1 + g0i2*g1 + g0i3*g1 + g0i4*g1 + g0i5*g1 + g0i6*g1
           y15_Q470_y15_Q75      := g0i0*g2 + g0i1*g2 + g0i2*g2 + g0i3*g2 + g0i4*g2 + g0i5*g2 + g0i6*g2
           y15_Q470_anxiety      := g0i0*g3 + g0i1*g3 + g0i2*g3 + g0i3*g3 + g0i4*g3 + g0i5*g3 + g0i6*g3
           y15_Q470_fatigue      := g0i0*g4 + g0i1*g4 + g0i2*g4 + g0i3*g4 + g0i4*g4 + g0i5*g4 + g0i6*g4
           y15_Q470_stress       := g0i0*g5 + g0i1*g5 + g0i2*g5 + g0i3*g5 + g0i4*g5 + g0i5*g5 + g0i6*g5
           y15_Q470_absenteeism  := g0i0*g6 + g0i1*g6 + g0i2*g6 + g0i3*g6 + g0i4*g6 + g0i5*g6 + g0i6*g6
           y15_Q470_presenteeism := g0i0*g7 + g0i1*g7 + g0i2*g7 + g0i3*g7 + g0i4*g7 + g0i5*g7 + g0i6*g7
           y15_Q470_headeye      := g0i0*g8 + g0i1*g8 + g0i2*g8 + g0i3*g8 + g0i4*g8 + g0i5*g8 + g0i6*g8

           y15_Q46_r0_WHO5         := z0i0*z1 + z0i1*z1 + z0i2*z1 + z0i3*z1 + z0i4*z1 + z0i5*z1 + z0i6*z1
           y15_Q46_r0_y15_Q75      := z0i0*z2 + z0i1*z2 + z0i2*z2 + z0i3*z2 + z0i4*z2 + z0i5*z2 + z0i6*z2
           y15_Q46_r0_anxiety      := z0i0*z3 + z0i1*z3 + z0i2*z3 + z0i3*z3 + z0i4*z3 + z0i5*z3 + z0i6*z3
           y15_Q46_r0_fatigue      := z0i0*z4 + z0i1*z4 + z0i2*z4 + z0i3*z4 + z0i4*z4 + z0i5*z4 + z0i6*z4
           y15_Q46_r0_stress       := z0i0*z5 + z0i1*z5 + z0i2*z5 + z0i3*z5 + z0i4*z5 + z0i5*z5 + z0i6*z5
           y15_Q46_r0_absenteeism  := z0i0*z6 + z0i1*z6 + z0i2*z6 + z0i3*z6 + z0i4*z6 + z0i5*z6 + z0i6*z6
           y15_Q46_r0_presenteeism := z0i0*z7 + z0i1*z7 + z0i2*z7 + z0i3*z7 + z0i4*z7 + z0i5*z7 + z0i6*z7
           y15_Q46_r0_headeye      := z0i0*z8 + z0i1*z8 + z0i2*z8 + z0i3*z8 + z0i4*z8 + z0i5*z8 + z0i6*z8



         #Direct effects
           computer0_WHO5         := h1i0 + h1i1 + h1i2 + h1i3 + h1i4 + h1i5 + h1i6
           computer0_y15_Q75      := h2i0 + h2i1 + h2i2 + h2i3 + h2i4 + h2i5 + h2i6
           computer0_anxiety      := h3i0 + h3i1 + h3i2 + h3i3 + h3i4 + h3i5 + h3i6
           computer0_fatigue      := h4i0 + h4i1 + h4i2 + h4i3 + h4i4 + h4i5 + h4i6
           computer0_stress       := h5i0 + h5i1 + h5i2 + h5i3 + h5i4 + h5i5 + h5i6
           computer0_absenteeism  := h6i0 + h6i1 + h6i2 + h6i3 + h6i4 + h6i5 + h6i6
           computer0_presenteeism := h7i0 + h7i1 + h7i2 + h7i3 + h7i4 + h7i5 + h7i6
           computer0_headeye      := h8i0 + h8i1 + h8i2 + h8i3 + h8i4 + h8i5 + h8i6
         ...
         #Total effects
          ...
          #By ICT mobile group
           ...
           indirect_self_alwaysoffice0_WHO5         := a0i6*a1 + b0i6*b1 + c0i6*c1 + d0i6*d1 + e0i6*e1 + f0i6*f1 + g0i6*g1
           indirect_self_alwaysoffice0_y15_Q75      := a0i6*a2 + b0i6*b2 + c0i6*c2 + d0i6*d2 + e0i6*e2 + f0i6*f2 + g0i6*g2
           indirect_self_alwaysoffice0_anxiety      := a0i6*a3 + b0i6*b3 + c0i6*c3 + d0i6*d3 + e0i6*e3 + f0i6*f3 + g0i6*g3
           indirect_self_alwaysoffice0_fatigue      := a0i6*a4 + b0i6*b4 + c0i6*c4 + d0i6*d4 + e0i6*e4 + f0i6*f4 + g0i6*g4
           indirect_self_alwaysoffice0_stress       := a0i6*a5 + b0i6*b5 + c0i6*c5 + d0i6*d5 + e0i6*e5 + f0i6*f5 + g0i6*g5
           indirect_self_alwaysoffice0_absenteeism  := a0i6*a6 + b0i6*b6 + c0i6*c6 + d0i6*d6 + e0i6*e6 + f0i6*f6 + g0i6*g6
           indirect_self_alwaysoffice0_presenteeism := a0i6*a7 + b0i6*b7 + c0i6*c7 + d0i6*d7 + e0i6*e7 + f0i6*f7 + g0i6*g7
           indirect_self_alwaysoffice0_headeye      := a0i6*a8 + b0i6*b8 + c0i6*c8 + d0i6*d8 + e0i6*e8 + f0i6*f8 + g0i6*g8
           ...
           direct_self_alwaysoffice0_WHO5          := h1i6
           direct_self_alwaysoffice0_y15_Q75       := h2i6
           direct_self_alwaysoffice0_anxiety       := h3i6
           direct_self_alwaysoffice0_fatigue       := h4i6
           direct_self_alwaysoffice0_stress        := h5i6
           direct_self_alwaysoffice0_absenteeism   := h6i6
           direct_self_alwaysoffice0_presenteeism  := h7i6
           direct_self_alwaysoffice0_headeye       := h8i6
...
df2$Mediator[df2$Mediator=="QD"] <- "Quantitative demands"
df2$Mediator[df2$Mediator=="freq_dis_int"] <- "Frequent disruptive interruptions"
df2$Mediator[df2$Mediator=="CD"] <- "Cognitive demands"
df2$Mediator[df2$Mediator=="computer"] <- "Computer (direct)"

df2$Mediator <- factor(df2$Mediator, levels=c("Weekly working hours","Work in free time","Quantitative demands","Frequent disruptive interruptions",
                                    "Take hour off","Cognitive demands","Autonomy","Social support","Computer (direct)"))
df2$Mediator <- factor(df2$Mediator, levels=rev(levels(df2$Mediator)))
...
ggsave("Effect of ICT-mobile work use on health indicators.eps", 
       plot = p + labs(title=NULL,caption=NULL, subtitle=NULL), 
       device = "eps", path="charts/",
       scale = 1, width = 16, height = 6, units = "cm")
write.csv(df, file="charts/Effect of ICT-mobile work use on health indicators.csv")
...
df3$group[df3$group=="teleworker"] <- "Home-based teleworker, employee"
df3$group[df3$group=="lowmob"] <- "Occasional TICTM, employee"
df3$group[df3$group=="highmob"] <- "High mobile TICTM, employee"
df3$group[df3$group=="alwaysofficeICT"] <- "Always at employers premise with ICT, employee"
df3$group[df3$group=="alwaysoffice"] <- "Always at employers premise w/o ICT, employee"
df3$group[df3$group=="selfICTM"] <- "Self-employed TICTM"
df3$group[df3$group=="self_alwaysoffice"] <- "Self-employed - Fixed place"

df3$group <- factor(df3$group, levels=c("Always at employers premise w/o ICT, employee","Always at employers premise with ICT, employee","Home-based teleworker, employee","High mobile TICTM, employee","Occasional TICTM, employee","Self-employed - Fixed place","Self-employed TICTM"))
...
p <-  ggplot(df3, aes(x=group, y=est.std, fill=effect)) + 
        facet_wrap( ~ Outcome , nrow=2  ) +
        geom_bar(stat="identity") + 
        scale_fill_manual("Effect", values=EF_2c) +
        labs(title = "Effect of ICT mobile work",
             ...
             y = "Standardised coefficient") +
        coord_flip() + 
        scale_y_continuous(expand = c(0, 0), limit = c(-0.05,0.08), breaks = seq(-0.03, 0.06, 0.03),labels = number_format(accuracy=0.01)) +
        scale_x_discrete(limits = rev(levels(df3$group))) +
        theme_minimal(base_size=10)  + 
...
ggsave("Effect of ICT mobile work.eps", 
       plot = p + labs(title=NULL,caption=NULL, subtitle=NULL), 
       device = "eps", path="charts/",
       scale = 1, width = 16, height = 8, units = "cm")
write.csv(df3, file="charts/Effect of ICT mobile work.csv")
...
p <- ggplot(subset(df2,is.na(x)), aes(x=Mediator, y=est.std)) + 
        facet_wrap( ~ Outcome , nrow=2) +
        geom_bar(stat="identity", fill=EF_orange) + 
        geom_errorbar(aes(ymin=ci.lower, ymax=ci.upper), colour="black", width=.3, size=0.2) +
...
ggsave("Effect of ICT mobile work mediated through job demands and resources.eps", 
       plot = p + labs(title=NULL,caption=NULL, subtitle=NULL), 
       device = "eps", path="charts/",
       scale = 1, width = 16, height = 8, units = "cm")
write.csv(df, file="charts/Effect of ICT mobile work mediated through job demands and resources.csv")
